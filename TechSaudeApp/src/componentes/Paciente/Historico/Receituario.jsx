...
const Receituario = () => {
  ...
  useEffect(() => {
    const fetchReceitas = async () => {
      try {
        const response = await axios.get(`${apiUrl}/api/Documento/receitas`, {
          params: {
        ...
        if (response.data && response.data.data && Array.isArray(response.data.data.$values)) {
          setReceitas(response.data.data.$values);
        } else {
          console.error('Dados inesperados recebidos:', response.data);
          setReceitas([]); // Define exames como vazio se a estrutura não estiver correta
        }
      } catch (error) {
        console.error('Erro ao buscar exames:', error);
        setReceitas([]); // Limpa a lista de exames em caso de erro
      }
    };
  ...
  const handleAddReceita = async () => {
    ...
    uploadFormData.append('categoria', 'Receitas');
  ...
  const handleDownload = async (receita) => {
    try {
      const response = await axios.get(`${apiUrl}/api/Documento/download/${receita.documentoID}`, {
      ...
      const fileName = receita.fileName ? `${receita.fileName}${extension}` : `arquivo${extension}`;
      ...
      <div className="historico-main-content">
        ...
        <div className="historico-content">
          <div className="historico-buttons">
            ...
            >
              <FontAwesomeIcon icon={faPlus} /> Adicionar Receituário
            </button>
          </div>
            ...
            <tbody>
              {receitas.length === 0 ? (
                ...
                receitas.map((receita) => (
                  <tr key={receita?.documentoID || 'default-key'}>
                    <td>{receita?.documentoID || 'N/A'}</td>
                    <td>{receita?.descricao || 'N/A'}</td>
                    <td>{formatData(receita?.dataAssociada)}</td>
                    <td className="historico-acao">
                      <button onClick={() => handleDownload(receita)} className="historico-botao-download" >
          ...
          <Modal show={showModal} onHide={() => setShowModal(false)}>
            <Modal.Header closeButton>
              <Modal.Title>Adicionar Receituario</Modal.Title>
            </Modal.Header>
            <Modal.Body>
            ...
            <Modal.Footer>
              <Button variant="primary" onClick={handleAddReceita}>
                Adicionar
              </Button>
            </Modal.Footer>
          </Modal>
